...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The two retrieval metrics stay out of this average on purpose: they
        diagnose the retriever, not the answer.
        """
...
def _clamp(score: float) -> float:
    """Keep every metric inside the documented [0.0, 1.0] range."""
...
class RAGASEvaluator:
    ...
    def evaluate_faithfulness(self, answer: str, context: str) -> float:
        ...
        return _clamp(grounded / len(answer_tokens))
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        if not question_tokens:
            return 1.0
        addressed = len(_tokenize(answer) & question_tokens)
        return _clamp(addressed / len(question_tokens))
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        if not expected_tokens:
            return 1.0
        covered = len(_tokenize(answer) & expected_tokens)
        return _clamp(covered / len(expected_tokens))
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        covered = len(expected_tokens & union_tokens)
        return _clamp(covered / len(expected_tokens))
    ...
    def evaluate_context_precision(
        ...
        for rank, chunk in enumerate(contexts, start=1):
            ...
            if coverage < relevance_threshold:
                continue
            relevant_seen += 1
            precision_sum += relevant_seen / rank  # Precision@k at this rank
...
class LLMJudge:
    ...
    def __init__(self, judge_llm_fn: Callable[[str], str]) -> None:
        # Injected so unit tests can pass a mock and never spend an API call.
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator of an AI assistant's answer.\n"
            "Score the answer against EACH criterion below on a 0.0-1.0 scale.\n"
            "Judge only the content: length is not a quality signal.\n\n"
            f"QUESTION:\n{question}\n\n"
            f"ANSWER:\n{answer}\n\n"
            f"RUBRIC:\n{criteria_lines}\n\n"
            "Reply with JSON only, mapping each criterion name to its score, "
            'e.g. {"accuracy": 0.8}.'
        )
        ...
        scores = {
            criterion: self._normalize_score(parsed.get(criterion))
            for criterion in rubric
        }
        return {"scores": scores, "reasoning": raw_response}

    @staticmethod
    def _parse_scores(raw_response: str) -> dict[str, Any]:
    ...
    def _normalize_score(value: Any) -> float:
        """Unparseable criteria default to 0.5; 1-5 rubric points map to 0-1."""
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            return 0.5
        score = float(value)
        if score > 1.0:
            score = score / 5.0  # judge answered on the 1-5 rubric scale
        return _clamp(score)
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        def mean_score(entry: Any) -> float | None:
            values = [
                float(value)
                for value in (entry or {}).get("scores", {}).values()
                if isinstance(value, (int, float)) and not isinstance(value, bool)
            ]
            return sum(values) / len(values) if values else None

        per_entry = [mean_score(entry) for entry in scores_batch]
        observed = [value for value in per_entry if value is not None]
        if not observed:
            return {
        ...
        overall_avg = sum(observed) / len(observed)

        # Positional bias: the answer shown FIRST scores clearly above the rest.
        positional_bias = False
        if len(observed) >= 2 and per_entry[0] is not None:
            rest = [value for value in per_entry[1:] if value is not None]
            if rest:
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # No retrieval trace recorded => keep the retrieval metrics None
            # instead of scoring an empty chunk list as a real 0.0.
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        for failure in failures:
            label = failure.failure_type or "unclassified"
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Several metrics tied at the bottom, or everything failing at once:
        # no single stage explains it, so send it back to the full pipeline.
        if len(weakest) > 1 or all(value < 0.5 for value in scores.values()):
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        Suggestions are matched by position; a missing one becomes "-".
        """
        lines = [
            "| Failure ID | Type | Root Cause | Suggested Fix | Status |",
            "|------------|------|------------|---------------|--------|",
    ...
    def generate_improvement_suggestions(
        ...
        playbook = {
            "hallucination": (
                "Implement a hallucination checker that drops answer sentences "
                "with no supporting retrieved chunk, and require source citations"
            ),
            "irrelevant": (
                "Rewrite the system prompt to restate the user intent before "
                "answering, and add query rewriting for short/ambiguous questions"
            ),
            "incomplete": (
                "Increase chunk size and top-k so conditions and exceptions stay "
                "together, and add few-shot examples of complete answers"
            ),
            "off_topic": (
                "Add intent detection plus a scope guard so out-of-scope questions "
                "are redirected instead of answered"
            ),
            "refusal": (
                "Loosen over-strict guardrails so in-scope questions are answered "
                "instead of refused"
            ),
        }
        ...
        # Most frequent failure type first — fix what hurts the most cases.
        ...
        # Retrieval evidence is only available when a trace was recorded.
        recalls = [f.context_recall for f in failures if f.context_recall is not None]
        if recalls and sum(recalls) / len(recalls) < 0.6:
            suggestions.append(
                "Fix retrieval first: average Context Recall is low, so the "
                "evidence never reaches the generator (tune chunking and top-k)"
            )
        precisions = [
        ...
        if precisions and sum(precisions) / len(precisions) < 0.6:
            suggestions.append(
                "Add a reranking step so relevant chunks appear before noise "
                "and stop diluting the context window"
            )

        fallbacks = [
            "Add the failing questions to the golden dataset and re-run the "
            "benchmark after every prompt change",
            "Calibrate the LLM judge against human labels before trusting the "
            "scores as a deployment gate",
            "Split long policy documents by section so each chunk keeps its "
            "conditions and effective dates intact",
        ]
